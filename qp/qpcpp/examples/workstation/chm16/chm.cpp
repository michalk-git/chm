...
#define PRINT_LOG(x) 
#else
CoreDebug print_handler(singleton<CoreDebug>::getInstance());
#define PRINT_LOG(x) do {print_handler.CorePrintf(x)} while (0)
#endif
...
namespace Core_Health {
	...
	Q_STATE_DEF(HealthMonitor, active) {
		...
		case KICK_SIG: {
			...
			else subscription_handler.LogUnResponsiveUsersAndReset();
