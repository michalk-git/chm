...
void Process(Request* r) {
	...
	case REGISTER: {
		...
		//else add the key-value pair of id-index to the id_to_index dictionary, increase the users count and print out the user's id
			...
			ae->id = id;
			Core_Health::AO_CHM->postFIFO(ae,&x);
			*r = FREE;
			id = 0;
		}
		break;
	...
	case SUBSCRIBE: {
		...
		Core_Health::AO_Member[index]->postFIFO(Q_NEW(QEvt, Core_Health::SUBSCRIBE_SIG),&x);
		break;
	}
	case UNSUBSCRIBE: {
		if ((c = QF_consoleGetKey()) != 0) {
		...
		Core_Health::AO_Member[index]->postFIFO(Q_NEW(QEvt, Core_Health::UNSUBSCRIBE_SIG),&x);
		break;
	}
	case MALFUNCTION: {
		if ((c = QF_consoleGetKey()) != 0) {
	...
	case MALFUNCTION_GET_MISSES: {
		if ((c = QF_consoleGetKey()) != 0) {
			...
			e->memberNum = index;
			e->period_num = misses;
			Core_Health::AO_Member[index]->postFIFO(e,&x);
			*r = FREE;
